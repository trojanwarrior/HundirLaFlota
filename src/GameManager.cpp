...
void
GameManager::start(GameState* state)
{
...
GameManager::loadResources()
{
  ...
  while (sI.hasMoreElements ())
  {
    ...
    for (i = settings->begin (); i != settings->end (); ++i)
    {
      typestr = i->first;
      datastr = i->second;
      Ogre::ResourceGroupManager::getSingleton ().addResourceLocation (datastr, typestr,
								       sectionstr);
    }
  }
}
...
GameManager::configure()
{
  ...
  _renderWindow = _root->initialise (true, "HUNDIR LA FLOTA"); //Creamos la ventana/marco donde se alojará el viewport (lienzo) para pintar
