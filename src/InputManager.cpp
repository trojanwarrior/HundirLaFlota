...
template<>
  InputManager* Ogre::Singleton<InputManager>::msSingleton = 0;

InputManager::InputManager () :
    _inputSystem (0), _keyboard (0), _mouse (0)
{
}
...
void
InputManager::initialise (Ogre::RenderWindow *renderWindow)
{
  if (!_inputSystem)
  {
...
void
InputManager::addKeyListener (OIS::KeyListener *keyListener, const std::string& instanceName)
{
  if (_keyboard)
  {
    ...
    else
    {
      ...
      std::cout << "addKeyListener ELEMENTO DUPLICADO NO HACER NADA" << std::endl;
    }
  }
}

void
InputManager::addMouseListener (OIS::MouseListener *mouseListener, const std::string& instanceName)
{
  if (_mouse)
  {
    // Comprobar si el listener existe.
    ...
    {
      // Elemento duplicado; no hacer nada.
      std::cout << "addMouseListener ELEMENTO DUPLICADO NO HACER NADA" << std::endl;
    }
  }
}
...
InputManager::removeKeyListener (const std::string& instanceName)
{
  ...
  {
    // No hacer nada.
    std::cout << "removeKeyListener NO ENCONTRADO NO HACER NADA" << std::endl;
  }
}

void
InputManager::removeMouseListener (const std::string& instanceName)
{
  // Comprobar si el listener existe.
  itMouseListener = _mouseListeners.find (instanceName);
  if (itMouseListener != _mouseListeners.end ())
  ...
  {
    // No hacer nada.
    std::cout << "removeMouseListener NO ENCONTRADO NO HACER NADA" << std::endl;
  }
}

void
InputManager::removeKeyListener (OIS::KeyListener *keyListener)
{
...
void
InputManager::removeMouseListener (OIS::MouseListener *mouseListener)
{
...
void
InputManager::setWindowExtents (int width, int height)
{
...
bool
InputManager::keyPressed (const OIS::KeyEvent &e)
{
  itKeyListener = _keyListeners.begin ();
  itKeyListenerEnd = _keyListeners.end ();
...
bool
InputManager::keyReleased (const OIS::KeyEvent &e)
{
  itKeyListener = _keyListeners.begin ();
  itKeyListenerEnd = _keyListeners.end ();
  // Delega en los KeyListener añadidos.
...
bool
InputManager::mouseMoved (const OIS::MouseEvent &e)
{
  itMouseListener = _mouseListeners.begin ();
  itMouseListenerEnd = _mouseListeners.end ();
...
bool
InputManager::mousePressed (const OIS::MouseEvent &e, OIS::MouseButtonID id)
{
  itMouseListener = _mouseListeners.begin ();
  itMouseListenerEnd = _mouseListeners.end ();
  // Delega en los MouseListener añadidos.
...
bool
InputManager::mouseReleased (const OIS::MouseEvent &e, OIS::MouseButtonID id)
{
  itMouseListener = _mouseListeners.begin ();
  itMouseListenerEnd = _mouseListeners.end ();
  // Delega en los MouseListener añadidos.
